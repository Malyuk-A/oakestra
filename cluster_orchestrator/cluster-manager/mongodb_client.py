...
def mongo_update_jobs_status(TIME_INTERVAL):
    "If there are no updates from a job in the last TIME_INTERVAL mark it as failed unless the job is completed"
    jobs = mongo_find_all_jobs()
    for job in jobs:
        try:
            updated = False
            for instance in range(len(job["instance_list"])):
                ...
                job_is_inactive =  last_time_job_was_modified < (datetime.now().timestamp() - TIME_INTERVAL) 
